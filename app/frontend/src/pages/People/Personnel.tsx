...
function toInput(row: Personnel): PersonnelInput {
  return {
    ...
    phone_number: cleanNullable(row.phone_number),
    address: cleanNullable(row.address),
    city: cleanNullable(row.city),
    province: row.province?.trim().toUpperCase() || undefined,
...
function validate(row: Personnel): string[] {
  ...
  if (row.postal_code && !POSTAL_CODE.test(row.postal_code.trim()))
